...
module DocsHelper
  def api_example_urls(url_method, *args)
    ...
    if json_args.empty? || json_args.none? { |h| h.is_a?(Hash) }
  ...
  def api_example(url_method, *)
    json_url, url = api_example_urls(url_method, *)
  ...
  def api_example_link(http_method, url_method, *)
    json_path, path = api_example_urls(url_method, *)
