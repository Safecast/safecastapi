...
class DeviceStoryCommentsController < ApplicationController
    ...
    @device_story_comment.user_id = current_user.id
      respond_to do |format|
        if @device_story_comment.save && !@device_story_comment.spam?
          ...
          flash[:error] = 'Could not submit your comment: ' + @device_story_comment.errors.full_messages.join(" ") + '!'
  ...
  def update
    if user_signed_in?
      if (@device_story_comment.user_id == current_user.id) || moderator?(current_user)
        respond_to do |format|
