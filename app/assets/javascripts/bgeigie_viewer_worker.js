...
self.onmessage = function(e)
{
    ...
    {
        self.ParseLogFileToVectors(e.data.log, e.data.logbuffer, e.data.userData, e.data.is_more_fields, e.data.logId, e.data.worker_id);
    }//if
    ...
    {
        self.SortByQuadKey(e.data.mxs, e.data.mys, e.data.minzs, e.data.cpms, e.data.times, e.data.alts, e.data.degs, e.data.logids, e.data.litcpms, e.data.litcp5s, e.data.valids, e.data.noproc, e.data.userData, e.data.isMobile, e.data.worker_id);
    }
    else
    {
...
self.SortByQuadKey = function (bmxs, bmys, bminzs, bcpms, btimes, balts, bdegs, blogids, blitcpms, blitcp5s, bvalids, noproc, userData, isMobile, worker_id)
{
    ...
    self.postMessage({op:"ORDER_BY_QUADKEY_ASC", userData:userData, worker_id:worker_id, 
                      mxs:mxs.buffer, mys:mys.buffer, minzs:minzs.buffer, times:times.buffer, cpms:cpms.buffer, alts:alts.buffer, degs:degs.buffer, logids:logids.buffer, litcpms:litcpms.buffer, litcp5s:litcp5s.buffer, valids:valids.buffer },
                      [   mxs.buffer,     mys.buffer,       minzs.buffer,       times.buffer,      cpms.buffer,      alts.buffer,      degs.buffer,        logids.buffer,         litcpms.buffer,         litcp5s.buffer,        valids.buffer ]);
};
...
self.ParseLogFileToVectors = function(log, logbuffer, userData, isMoreFields, logId, worker_id)
{
    ...
    var s_header, s_id, s_time, s_cpm, s_cp5s, s_totc, s_rnStatus, s_latitude, s_northsouthindicator, 
        s_longitude, s_eastwestindicator, s_altitude, s_gpsStatus, s_dop, s_quality, s_checksum;
                ...
                && s_longitude != null && s_longitude.length > 0)
            {
                ...
                if (blat > -85.05112878 && blat < 85.05112878 && blon >= -180.0 && blon <= 180.0)
                {
                    ...
                    if (destlitcpms != null)
                    {
                        ...
                        destvalids[idx]  = ((s_rnStatus  == "A"                    ? 1 : 0) << 2)
                                         | ((s_gpsStatus == "A"                    ? 1 : 0) << 1)
    ...
    if (idx > 0)
    {
        ...
        self.AssignScaleVisibilityToVecs(destmxs, destmys, destminzs, destcpms, destlitcpms, destlitcp5s, destvalids, idx, worker_id);
    ...
    self.postMessage({op:"PARSE_LOG_TO_VEC", ex:ex, userData:userData, worker_id:worker_id, 
                      mxs:destmxs.buffer, mys:destmys.buffer, minzs:destminzs.buffer, times:desttimes.buffer, cpms:destcpms.buffer, alts:destalts.buffer, degs:destdegs.buffer, litcpms:destlitcpms.buffer, litcp5s:destlitcp5s.buffer, valids:destvalids.buffer },
                      [   destmxs.buffer,     destmys.buffer,       destminzs.buffer,       desttimes.buffer,      destcpms.buffer,      destalts.buffer,      destdegs.buffer,         destlitcpms.buffer,         destlitcp5s.buffer,        destvalids.buffer ]);
};
...
{
    var cpm,z,zdest,i,j,zdest_n=0,match,z_diff,mx,my,p,did_replace;
...
function vcmprs_u08(s,x,g,nn,sn) { var d=new   Uint8Array(nn);var j=0;for (var i=0;i<sn;i++)if(g[i]!=x)d[j++]=s[i];return d; }
...
function vtrunc_u08(s,n) { var d=new   Uint8Array(n);d.set(s.subarray(0,n));return d; }
...
function PointDistanceMeters_EPSG4326(lat0, lon0, lat1, lon1) 
{
    ...
    if (lat0 == 0.0 || lon0 == 0.0 || lat1 == 0.0 || lon1 == 0.0 || lat0 == -9000.0 || lon0 == -9000.0 || lat1 == -9000.0 || lon1 == -9000.0) return 0.0; // sanity check input
